...
main = do
  ...
  defaultMain $ testGroup "hashes"
    [ testGroup "16bit"
      ...
      , testProperty "sh16"       $ avalanche16 simpleHash16 ==~ 0.0003663
      , testProperty "th16"       $ avalanche16 tabulation16 ==~ 0.0053710
      ]
    , testGroup "32bit"
...
tabulation16 w =
  ...
  where
    aux :: Word16 -> Word16
    aux idx = indexPrimArray table16 (fromIntegral idx)
...
tabulation32 w0 =
  ...
  where
    aux :: Word32 -> Word32
    aux idx = indexPrimArray table32 (fromIntegral (idx .&. 0xff))
    {-# INLINE aux #-}

    aux' :: Word32 -> Word32
    aux' idx = indexPrimArray table32 (fromIntegral idx)
    {-# INLINE aux' #-}
